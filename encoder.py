...
class selfatt(nn.Module):
    ...
    def forward(self,inputs,final_state,mask):#b,s,h;b,h;b,s
        ...
        return fuse,att
class mulatts(nn.Module):
    def __init__(self,config):
        """Base RNN Encoder Class"""
        super(mulatts, self).__init__()
        self.config=config
        self.linear1=torch.nn.Linear(2*config.hidden_size,2*config.hidden_size,bias=False)
        self.context_lstm = ContextRNN(config.hidden_size * 4, config.hidden_size)
#        self.linear2=torch.nn.Linear(hidden_size,1)
    def forward(self,inputs,mask,length):#b,c,s,h;b,c,s;b,c
#        print(inputs.size())
#        print(mask.size())
#        print(selfatt.size())
#         print(inputs.size())
...
#         print(fused.size())
        ...
        fused,_=torch.max(fused,-2)
        # fused=torch.cat([fused,inputs2])
        # length=length.unsqueeze(-1).repeat(1,1,context_size)
        # _,(h,_)=self.context_lstm(fused.reshape(-1,sentence_size,4*self.config.hidden_size),length.reshape([-1]))
        # h=h.transpose(0,1)
        # h=h.reshape([-1,context_size,context_size,2*self.config.hidden_size])
        # fused=torch.matmul(selfatt,fused).squeeze(-2)#b,c2m,c1,h
        return fused,att,fuses
...
class EncoderRNN(BaseRNNEncoder):
    ...
    def forward(self, inputs, input_length, hidden=None):
        ...
        embedded = self.dropout(self.embedding(inputs_sorted))
