...
def load_data(input_file, maxdoc=30, maxword=50):
    ...
    for i in range(pred_pair.shape[0]):
        for j in range(pred_pair.shape[1]):
            if pred_pair[i,j]>0.5:
                pairs.append((i+1,j+1))
    return pairs
def evaluate(model,test_iter,show=False):
    model.eval()
...
#                a=input('hahah')
            ...
            n2s+=n2
            n3s+=n3
            if n1==n2 and n2==n3 and logits_for_class[idx][:cur_length].tolist()==item['labels']:
                em+=1
                if(show):
                    tmp1=att1[idx]
                    tmp2=att2[idx]
...
#                                    a=input('haha')
                ...
                errorr_esults.append([content,truelabel,predlabel,truetuple,predtuple])
            nums+=1
